"""
This script assigns gates to a partitioned genetic circuit using simulated annealing 

At each timestep, a gate from a randomly chosen cell is swapped with another gate within the same cell, or with an unused gate from the library. 
If the circuit score improves, swap is accepted; if not, it is accepted based on certain probability 

Note: circuit score does not take gate toxicity into consideration yet, a new function needs to be added 

Author: Jing Zhang (jillwill00@gmail.com)
"""
...
#########################################
# Load graph and partition result
#########################################

def load_graph (edgelist):
	""" 
	read DAG edgelist, return DIRECTED graph
	"""
...
def calc_circuit_score (assignedLib, gateLib, sensorLib):
	"""
	calculate circuit score for each temporary gate assignment 
	"""
...
def assign_gates (G, partition, params, path):
	"""
	Assign biological gates to partitioned solution using simulated annealing
	"""
	# assign biological gates to partitioned graphs
	gateLib = load_gate_info('./lib/gatefunction.txt')
	sensorLib = load_sensor_info('./lib/sensor.txt')
	# get input, output, and non primitive nodes in the graph 
	inodes, onodes, nonprimitives  = get_nonprimitive_nodes (G)
	G_primitive = get_G_primitive (G)  # remove input and output nodes 
	...
	if circuit_valid:
		...
		for i in range(5):  # for 100 trajectories
			...
			for t in range(1000000):   # perform simulated annealing
				...
				S, circuit_valid = calc_circuit_score (assignedLib_tmp, gateLib, sensorLib)
			...
			record_library (bestLib, path+'/optimal gate assignments_trajectory_'+str(i)+'.txt')
...
if __name__ == '__main__': 
	partition, params, n, k
	G = load_graph ('./example_graph/DAG.edgelist')   # load edgelist
	partition = load_part_sol ('./example_graph/part_soln.txt')  # load partition result 
	params = []    # define simulated annealing parameter here
	outpath = ''   # directory to which solution is saved into 
	assign_gates(G, partition, params, outpath)
	plot_circuit_score ('S_trajectory.txt')