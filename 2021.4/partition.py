...
def find_best_n(G, nonprimitives, primitive_only):
    ...
    if len(list(G_primitive.nodes())) > 7:
        ...
        nparts = list(
            range(
                int(len(G_primitive.nodes()) / max_nodes) + 1,
                int(len(G_primitive.nodes()) / min_nodes) + 1,
            )
        )
        # print('nparts', nparts)
...
def partition_mincut_coi(dag_file, nonprimitives, primitive_only, outdir):
    ...
    visualize_assignment_graphviz(
        DiG, outdir, part_opt, nonprimitives, primitive_only, outfig
    )
...
def partition_nparts_coi(dag_file, nonprimitives, primitive_only, outdir):
    ...
    if len(list(G_primitive.nodes())) > 7:
        ...
        for n in nparts:
            ...
            DiG = nx.read_edgelist(dag_file, nodetype=str, create_using=nx.DiGraph)
            visualize_assignment_graphviz(
                DiG, outdir, part_opt, nonprimitives, primitive_only, outfile
            )


def visualize_assignment_graphviz(
    G, outdir, partition, nonprimitives, primitive_only, outfile
):
    ...
    for inode in innodes:
        for outnode in outnodes:
            for path in sorted(nx.all_simple_edge_paths(dag, inode, outnode)):
                if all(
                    e in list(dag.edges()) for e in path
            ...
            for partcombo in parts_to_move:
                ...
                if valid:
                    ...
                    if all(s <= Smax for s in part_sizes) and all(
                        ...
                        matrix, partG = partition_matrix(
                            G, part_opt_format, innodes + outnodes, primitive_only
                        )
                        # print(matrix)
...
def optimize_signal_simulated_annealing(
    G,
    innodes,
    outnodes,
    primitive_only,
            ...
            for t in range(1, 10000):  # timestep in simulated annealing
                ...
                while size_constraint == False:
                    ...
                    nodes_to_move = random.sample(
                        subG_nodes, random.choice(np.arange(1, 3))
                    )
                    # nodes_to_move = ['82']
                    # print(nodes_to_move)
                ...
                matrix_new, partG_new = partition_matrix(
                    G, part_opt_format_new, nonprimitives, primitive_only
                )
                ...
                part_opt_format_best = (
                    cut_bp,
                    [get_part(bestpartDict, n) for n in G_primitive.nodes()],
                )
                matrix_best, partG_best = partition_matrix(
                    G, part_opt_format_best, nonprimitives, primitive_only
                )
...
def plot_simulated_annealing_result(G, nonprimitives, primitive_only, outdir):
    ...
    for sol in [0]:
        part_opt_format = (
            solDict[sol]["cut"],
            [get_part(solDict[sol]["part"], n) for n in G_primitive.nodes()],
        )
        print(solDict[sol]["part"])
        visualize_assignment_graphviz(
            G,
            ...
            nonprimitives,
            primitive_only,
            outdir + "part_opt_trajectory" + str(sol + 1) + ".txt",
        )
...
def record_opt_part(G, path, part_opt, sol, nonprimitives, primitive_only, solN):
    ...
    visualize_assignment_graphviz(
        G, path, part_opt, nonprimitives, primitive_only, path + "/part_opt" + str(solN)
    )
...
def record_ori_part(G, path, part_opt, partDict, nonprimitives, T, cut):
    visualize_assignment_graphviz(
        G, path, part_opt, nonprimitives, primitive_only, "part_opt0"
    )
...
def get_optimal_results(
    G, G_primitive, cut, innodes, outnodes, primitive_only, partDict, constraint, path
):
    ...
    if os.path.exists(signal_optimized_file):
        ...
        minT = min([solDict[s]["T"] for s in solDict])
        minC = min(
            [solDict[s]["cut"] for s in solDict if solDict[s]["T"] == minT]
        )  # from all sols with minT, choose one with the lowest C
        # print('minT', minT, 'minC', minC)
        for s in solDict:
            # print('solution', s)
            # print(solDict[s])
            if solDict[s]["T"] == minT and solDict[s]["cut"] == minC:
                part_opt_format = (
                    cut,
                    [get_part(solDict[s]["part"], n) for n in G_primitive.nodes()],
                )
                record_opt_part(
                    G,
                    path,
        ...
        if os.path.exists(potential_sol_file):
            ...
            minC = min(
                [potsolDict[s]["cut"] for s in potsolDict if potsolDict[s]["T"] == minT]
            )  # from all sols with minT, choose one with the lowest C
            ...
            if solN == 1:
                ...
                if motif_allowed and loop_free:
                    ...
                    record_ori_part(
                        G, path, part_ori_format, partDict, nonprimitives, T, cut
                    )
...
if __name__ == "__main__":
    ...
    if os.path.exists(dag_file):
        ...
        if max(T) > 0:
            ...
            if solN == 0:
                solN_relaxed = optimize_signal(
                    dag, cut, partDict, 5, False, path + "/optimized_lc/"
                )
            ...
            if os.path.exists(path + "/optimized_lc"):
                ...
                get_optimal_results(
                    dag, G_primitive, cut, innodes, outnodes, partDict, False, path
                )
