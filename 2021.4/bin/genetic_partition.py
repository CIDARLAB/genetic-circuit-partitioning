    ...
    if len(G_primitive.nodes()) > Smax:
        ...
        for i in range(1, trajectories + 1):  # for given number of trajectories
            ...
            for t in range(1, 10000):  # timestep
                ...
                part_opt_format_best = (
                    cut_bp,
                    [get_part(bestpartDict, n) for n in G_nodes],
                )
                ...
                for cell in list(bestpartG.nodes()):
                    ...
                    subG_best_motif_allowed = check_motif_allowed(
                        subG_matrix_best, motif_constraint
                    )
                ...
                part_opt_format_new = (
                    cut_np,
                    [get_part(partDict_tmp, n) for n in G_primitive.nodes()],
                )
                ...
                try:
                    ...
                    subG_new_motif_allowed = check_motif_allowed(
                        subG_matrix_new, motif_constraint
                    )
...
def check_all_solutions(
    graph, G, nonprimitives, primitive_only, high_constraint, low_constraint, outdir
):
    """
        ...
        if npart.isdigit():
            ...
            for constraint in ["lc", "hc"]:
                ...
                solDict = load_opt_part_sol(
                    outdir + npart + "/optimized_" + constraint + "/solution.txt"
                )
                minTDict = load_minT_sol(
                    outdir + npart + "/optimized_" + constraint + "/minT.txt"
                )
                ...
                for iteration in solDict.keys():
                    ...
                    part_opt_format = (
                        cut,
                        [get_part(part, n) for n in G_primitive.nodes()],
                    )
                    ...
                    f_out.write(
                        "\t".join([str(graph), str(nodes), npart, constraint]) + "\t"
                    )
                    f_out.write(
                        "\t".join(
                            [
        ...
        for npart in nparts:
            if npart.isdigit():
                if os.path.exists(
                    ...
                    part_opt_o = (
                        cut_o,
                        [get_part(part_o, n) for n in G_primitive.nodes()],
                    )
                    matrix_o, partG_o = partition_matrix(G_primitive, part_opt_o)
                    motif_allowed_o = check_motif_allowed(matrix_o, motif_constraint)
                    loop_free_o = check_cycles(partG_o)
