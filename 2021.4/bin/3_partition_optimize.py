		...
		if target_n == ['']:
			...
			for n in target_n:
				...
				if len(list(G.nodes()))>=25:
					print('optimizing by subnetworks')
					print('optimizing with low constraint')
					gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, low_constraint, loop_free, priority, trajectories, outdir + n + '/optimized_lc/')
					# print('optimizing with high constraint')
					# gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, high_constraint, loop_free, priority, trajectories, outdir + n + '/optimized_hc/')
				else:
					print('brute-force optimizing')
				...
				## if partitioning into target_n doesn't have a solution, try splitting cells 
