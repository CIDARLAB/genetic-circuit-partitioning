...
def load_graph(settings, sample):
    ...
    """
    G = nx.read_edgelist(
        edgelist_filename(settings, sample), nodetype=str, create_using=nx.DiGraph()
    )
    return G
...
def load_opt_part_sol(inputfile):
    ...
    iteration_idx = [
        lines.index(line)
        for line in lines
        if line.startswith("path") or line.startswith("sol")
    ]
    solDict = {}
...
def read_json(inputfile):
    ...
    spacer = [
        idx + p_s + 1
        for idx, line in enumerate(lines[p_s + 1 : p_e])
        if ": {" in line.strip()
    ]
    ...
    spacer = [
        idx + g_s + 1
        for idx, line in enumerate(lines[g_s + 1 : g_e])
        if "$abc$" in line.strip()
    ]
    for i, v in enumerate(spacer):
        # get names
    ...
    for inode in in_nodes:
        for outnode in out_nodes:
            for path in sorted(nx.all_simple_edge_paths(G, inode, outnode)):
                if all(
                    e in list(G.edges()) for e in path
    ...
    for in_node in in_nodes:
        for out_node in out_nodes:
            for path in sorted(nx.all_simple_edge_paths(partG, in_node, out_node)):
                if all(
                    e in list(partG.edges()) for e in path
                ):  # valid path that makes a directed path from source to target
                    cross = len(path)
                    crosslist.append(cross)
    return crosslist
...
def custom_cycle_finder(G, source=None, orientation=None):
...
def check_cycles(partG):
    ...
    """
    return True if custom_cycle_finder(partG) else False
...
def partition_matrix(G: nx.DiGraph, partition):
    """
...
def get_subnetwork(matrix, cell):
    ...
    to_cell = matrix[:, cell]
    neighbors = [idx for idx, val in enumerate(from_cell) if val != 0] + [
        idx for idx, val in enumerate(to_cell) if val != 0
    ]
...
def visualize_assignment_graphviz(
    G, partition, nonprimitives, primitive_only, outdir, iteration
):
    """
...
def optimize_signal_subnetwork(
    G,
    primitive_only,
    S_bounds,
    cut,
    ...
    part_opt = [get_part(partDict, n) for n in G_nodes]
    matrix, partG = partition_matrix(G_primitive, part_opt)
    ...
    if len(G_primitive.nodes()) > Smax:
        ...
        for i in range(1, trajectories + 1):  # for given number of trajectories
            ...
            part_opt_format_best = [get_part(bestpartDict, n) for n in G_nodes]
            matrix_best, partG_best = partition_matrix(
                G_primitive, part_opt_format_best
            )
            ...
            for cell in list(partG_best.nodes()):
                ...
                subG_matrix_best, subG_partG_best = get_part_matrix_subG(
                    matrix_best, partG_best, subG_cells
                )
                subG_best_loop_free, subG_best_motif_allowed = check_constraint(
                    subG_matrix_best, subG_partG_best, motif_constraint
                )
            ...
            for t in range(1, 15000):  # timestep
                ...
                while size_constraint == False:
                    ...
                    subG_nodes = list(
                        set(
                            [
                                n
                                for n in G_nodes
                                if get_part(bestpartDict, n) in subG_cells
                            ]
                        )
                        - set(locked_nodes)
                    )
                    ...
                    while have_nodes_to_move == False:
                        try:
                            nodes_to_move = random.sample(
                                subG_nodes, random.choice(np.arange(1, maxNodes + 1))
                            )
                    ...
                    size_constraint = all(s <= Smax for s in part_sizes) and all(
                        s >= Smin for s in part_sizes
                    )
                ...
                part_opt_format_new = [get_part(partDict_tmp, n) for n in G_primitive.nodes()]
                matrix_new, partG_new = partition_matrix(
                    G_primitive, part_opt_format_new
                )
                try:
                    subG_matrix_new, subG_partG_new = get_part_matrix_subG(
                        matrix_new, partG_new, subG_cells
                    )
                    ...
                    subG_new_loop_free, subG_new_motif_allowed = check_constraint(
                        subG_matrix_new, subG_partG_new, motif_constraint
                    )
        ...
        for i in bestT_dict:
            f_out.write(
                str(i) + "\t" + ",".join([str(T) for T in bestT_dict[i]]) + "\n"
            )
            f_out3.write(
                str(i) + "\t" + ",".join([str(t) for t in timproved_dict[i]]) + "\n"
            )
            ...
            for part in bestpartDict_all[i]:
                f_out2.write(
                    ...
                    + str(part)
                    + "\t"
                    + ",".join(bestpartDict_all[i][part])
                    + "\n"
                )


def optimize_signal_bruteforce(
    G, primitive_only, S_bounds, cut, partDict, maxNodes, motif_constraint, outdir
):
    """
    ...
    if minT > 0:
        ...
        nodes_to_move = generate_combinations(
            list(G_primitive.nodes()), range(1, maxNodes + 1)
        )
        ...
        for nodescombo in nodes_to_move:
            ...
            parts_to_move = [
                ...
                for p in itertools.product(
                    list(partDict.keys()), repeat=len(nodescombo)
                )
            ...
            for partcombo in parts_to_move:
                ...
                if valid:
                    ...
                    if all(s <= Smax for s in part_sizes) and all(
                        s >= Smin for s in part_sizes
                    ):
...
def determine_best_solution(G, primitive_only, high_constraint, low_constraint, outdir):
    ...
    for constraint in ["lc", "hc"]:
        if constraint == "lc":
            motif_constraint = low_constraint
        else:
            motif_constraint = high_constraint
        for npart in nparts:
            if npart.isdigit():
                if os.path.exists(
                    outdir + npart + "/optimized_" + constraint + "/part_solns.txt"
                ):
                    # check if original partition satisfy constraints
                    cut_o, part_o = load_metis_part_sol(
                        outdir + npart + "/part_solns.txt"
                    )
                    T_o = max(calc_signal_path(G, in_nodes, out_nodes, part_o))
                    part_opt_o = [get_part(part_o, n) for n in G_primitive.nodes()]
                    matrix_o, partG_o = partition_matrix(G_primitive, part_opt_o)
                    motif_allowed_o = check_motif_allowed(matrix_o, motif_constraint)
                    loop_free_o = check_cycles(partG_o)
                    ...
                    for iteration in solDict.keys():
                        ...
                        # best soln so far
                        matrix_bs, partG_bs = partition_matrix(
                            G_primitive, best_soln[0][1]
                        )
                        loop_free_bs = check_cycles(partG_bs)
                        motif_allowed_bs = check_motif_allowed(
                            matrix_bs, motif_constraint
                        )

                        if not (
                            motif_allowed_bs and loop_free_bs
                        ):  # if best solution doesn't satisfy constraint
                    ...
                    for soln in best_soln:
                        ...
                        if loop_free_bs and motif_allowed_bs:
                            f_out.write(
                                "\t".join(
                                    [
                                        str(npart),
                                        str(soln[0]),
                                        str(len(list(G.nodes()))),
                                        constraint,
                                        str(motif_allowed_o),
                                        str(motif_allowed_bs),
                                        str(loop_free_o),
                                        str(loop_free_bs),
                                        str(T_o),
                                        str(minT),
                            ...
                            visualize_assignment_graphviz(
                                G,
                                soln[1],
                                nonprimitives,
                                primitive_only,
                                outdir + npart + "/optimized_" + constraint,
                                soln[0],
                            )
