...
def main():
    ...
    for s in samples:
        ...
        if target_n == [
            ""
        ]:  # if user doesn't supply a target number that the graph should be partitioned into
            nparts = list(
                range(
            ...
            for n in nparts:
                ...
                gp.visualize_assignment_graphviz(
                    DAG, part_opt, nonprimitives, primitive_only, outdir, 0
                )

        else:  # if user provides a target number
            os.mkdir(out_path + "/nparts/")
            os.mkdir(out_path + "/nparts/" + target_n)
            part_opt = gp.partition_nparts_wrapper(
                G_primitive, int(target_n), out_path + "/nparts/" + target_n
            )
            gp.visualize_assignment_graphviz(
                DAG,
                part_opt,
