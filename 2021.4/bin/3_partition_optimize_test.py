	...
	for s in samples:
		...
		if target_n == ['']:
			for npart in os.listdir(outdir):
				if npart.isdigit():
					...
					if len(list(G.nodes()))>=25:
						...
						gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, low_constraint, trajectories, outdir + npart +'/optimized_lc/')
						gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, high_constraint, trajectories, outdir + npart +'/optimized_hc/')
					else:                                
						...
						gp.optimize_signal_bruteforce (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, low_constraint, outdir + npart +'/optimized_hc/')
			...
			for n in target_n:
				...
				if len(list(G.nodes()))>=25:
					print('optimizing by subnetworks')
					gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, low_constraint, trajectories, outdir + n + '/optimized_lc/')
					# gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, high_constraint, trajectories, outdir + n + '/optimized_hc/')
				else:
					print('brute-force optimizing')
