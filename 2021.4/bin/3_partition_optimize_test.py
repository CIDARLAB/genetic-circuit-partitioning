...
def main():
	...
	graphviz.output_file = 'partition_test_276.png'
		...
		for s in samples:
			...
			if target_n == ['']:
				for npart in os.listdir(outdir):
					if npart.isdigit():
						...
						if len(list(G.nodes()))>=25:
							...
							gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, low_constraint, priority, trajectories, outdir + npart +'/optimized_lc/')
							print('optimizing with high constraint')
							gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, high_constraint, priority, trajectories, outdir + npart +'/optimized_hc/')
						else:                                
							...
							print('execution time', datetime.datetime.now()-begin_time)
				print('determining best solutions')
				...
				for n in target_n:
					...
					if len(list(G.nodes()))>=25:
						print('optimizing by subnetworks')
						gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, low_constraint, priority, trajectories, outdir + n + '/optimized_lc/')
						gp.optimize_signal_subnetwork (DAG, primitive_only, S_bounds, cut, partDict, maxNodes, high_constraint, priority, trajectories, outdir + n + '/optimized_hc/')
					else:
						print('brute-force optimizing')
