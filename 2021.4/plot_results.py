...
def count_opt_params(indir):
    ...
    cut_T_opt_num = 0
    f_out = open(
        "./boolean circuit/optimization improvement - 4 input (112220).txt", "w"
    )
    f_out.write(
        "Graph\tNodes\tValid Motif_METIS_lowconstraint\tValid Motif_METIS_highconstraint\tValid Motif_Optimized_lowconstraint\tValid Motif_Optimized_highconstraint\tCycle Free_METIS\tCycle Free_Optimized\tT_Metis\tT_Optimized\tT_Delta\tCut_Metis\tCut_Optimized\tCut_Delta\n"
    )
    ...
    for i in range(1, 3000):
        ...
        part_opt_format = (
            cut_o,
            [get_part(partDict_o, n) for n in G_primitive.nodes()],
        )
        matrix, partG = partition_matrix(G, part_opt_format, nonprimitives)
        cycle_free_o = check_cycles(partG)
        motif_valid_o_lc, motif_valid_o_hc = (
            check_motif_allowed(matrix, False),
            check_motif_allowed(matrix, True),
        )
        ...
        if os.path.exists(path + str(i) + "/optimized_hc/part_opt1.txt"):
            ...
            optfiles = [
                filename
                for filename in os.listdir(path + str(i) + "/optimized_hc")
                if filename.startswith("part_opt") and filename.endswith(".txt")
            ]
            for optfile in optfiles:
                T, cut, partDict = load_part_sol(
                    path + str(i) + "/optimized_hc/" + optfile
                )
                part_opt_format = (
                    cut,
                    [get_part(partDict, n) for n in G_primitive.nodes()],
                )
                matrix, partG = partition_matrix(G, part_opt_format, nonprimitives)
                if T < minT:
                    minT = T
            ...
            optfiles = [
                filename
                for filename in os.listdir(path + str(i) + "/optimized_lc")
                if filename.startswith("part_opt") and filename.endswith(".txt")
            ]
            for optfile in optfiles:
                T, cut, partDict = load_part_sol(
                    path + str(i) + "/optimized_lc/" + optfile
                )
                part_opt_format = (
                    cut,
                    [get_part(partDict, n) for n in G_primitive.nodes()],
        ...
        f_out.write(
            "\t".join(
                [
                    str(i),
                    str(node_num),
                    str(motif_valid_o_lc),
                    str(motif_valid_o_hc),
                    str(motif_valid_lc),
                    str(motif_valid_hc),
                    str(cycle_free_o),
                    str(cycle_free),
    ...
    f_out = open(
        "./boolean circuit/cut occur before optimization - 4 input (G1-3000).txt", "w"
    )
    f_out.write("Cut\tOccur\n")
    for idx, c in enumerate(x):
        f_out.write(str(c) + "\t" + str(occur[idx]) + "\n")
    ...
    f_out = open(
        "./boolean circuit/T occur before optimization - 4 input (G1-3000).txt", "w"
    )
    f_out.write("T\tOccur\n")
    ...
    f_out = open(
        "./boolean circuit/cut occur after optimization - 4 input (G1-3000).txt", "w"
    )
    f_out.write("Cut\tOccur\n")
    for idx, c in enumerate(x):
        f_out.write(str(c) + "\t" + str(occur[idx]) + "\n")
    ...
    f_out = open(
        "./boolean circuit/T occur after optimization - 4 input (G1-3000).txt", "w"
    )
    f_out.write("T\tOccur\n")
    for idx, t in enumerate(x):
        f_out.write(str(t) + "\t" + str(occur[idx]) + "\n")
...
def get_motif_occurence(path):
    ...
    for i in range(1, 65530):
        ...
        if os.path.exists(mfile):
            ...
            if os.path.exists(file_path + "/optimized_hc/part_opt1.npy"):
                optfiles = [
                    path + str(i) + "/optimized_hc/" + filename
                    for filename in os.listdir(path + str(i) + "/optimized_hc")
                    if filename.startswith("part_opt") and filename.endswith(".npy")
                ]
            elif os.path.exists(file_path + "/optimized_lc/part_opt1.npy"):
                optfiles = [
                    path + str(i) + "/optimized_lc/" + filename
                    for filename in os.listdir(path + str(i) + "/optimized_lc")
                    if filename.startswith("part_opt") and filename.endswith(".npy")
                ]
            else:
...
def calc_opt_results(inputfile):
    ...
    motif_valid_ori_lc = (
        [data[graph]["Valid Motif_METIS_lowconstraint"] for graph in data].count("True")
        * 100
        / len(data)
    )
    motif_valid_ori_hc = (
        [data[graph]["Valid Motif_METIS_highconstraint"] for graph in data].count(
            "True"
        )
        * 100
        / len(data)
    ...
    motif_valid_opt_lc = (
        [data[graph]["Valid Motif_Optimized_lowconstraint"] for graph in data].count(
            "True"
        )
        * 100
        / len(data)
    ...
    motif_valid_opt_hc = (
        [data[graph]["Valid Motif_Optimized_highconstraint"] for graph in data].count(
            "True"
        )
        * 100
        / len(data)
    ...
    cycle_free_ori = (
        [data[graph]["Cycle Free_METIS"] for graph in data].count("True")
        * 100
        / len(data)
    )
    cycle_free_opt = (
        [data[graph]["Cycle Free_Optimized"] for graph in data].count("True")
        * 100
        / len(data)
    )
...
def compare_cuts(infile1, infile2):
    ...
    (markerline, stemlines, baseline) = plt.stem(
        x1, y1, linefmt="#94b4a4", label="Initial Graph Partition"
    )
    (markerline2, stemlines2, baseline2) = plt.stem(
        x2, y2, linefmt="#c56183", label="Node Migration Optimized"
    )
    ax.set_xlabel("Cuts")
    ax.set_ylabel("Count")
    for spine in ["top", "right"]:
...
def calc_distance_to_boundry(G, part_dict, node, p):
    ...
    for n in nodes:
        ...
        others = set(nbs) - set(
            nodes
        )  # others: neighboring nodes that are not in the same block
        if len(others) != 0:
            dist = len(p[n][node]) - 1
    ...
    for n in data:
        ...
        for g in data[n]:
            ...
            if min(data[n][g]) not in [0, -1]:
                mindelta = min(
                    [
                        data[n][g][s]["New Cut"] - data[n][g][s]["Ori Cut"]
                        for s in data[n][g]
                    ]
                )
                for s in data[n][g]:
...
def count_num_moved_nodes(path):
    ...
    for i in range(1, 65530):
        ...
        if os.path.exists(path + str(i) + "/DAG.edgelist"):
            ...
            try:
                T_o, cut_o, partDict_o = load_part_sol(
                    path + str(i) + "/part_solns.txt"
                )
                ...
                if os.path.exists(path + str(i) + "/optimized_hc/part_opt1.txt"):
                    ...
                    for optfile in optfiles:
                        ...
                        # matrix, partG = partition_matrix (G, part_opt_format, nonprimitives)
                        num_moved_nodes = get_moved_node_property(
                            G_primitive, partDict_o, partDict
                        )
                    ...
                    f_out.write(
                        str(i)
                        + "\t"
                        + str(len(partDict.keys()))
                        + "\t"
                        + ",".join(num_moved_nodes_i)
                        + "\n"
                    )
...
def plot_num_moved_nodes(inputfile):
    ...
    x = np.arange(5)
    y1 = [
        v / sum([num_moved_nodes_dict[2][n] for n in range(1, 6)])
        for v in [num_moved_nodes_dict[2][n] for n in range(1, 6)]
    ]
    y2 = [
        v / sum([num_moved_nodes_dict[3][n] for n in range(1, 6)])
        for v in [num_moved_nodes_dict[3][n] for n in range(1, 6)]
    ]
    y3 = [
        v / sum([num_moved_nodes_dict[4][n] for n in range(1, 6)])
        for v in [num_moved_nodes_dict[4][n] for n in range(1, 6)]
    ]
    width = 0.2
