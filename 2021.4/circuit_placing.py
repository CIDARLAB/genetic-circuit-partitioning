...
def initialize_hexmap(n):
    ...
    for i in hexmap.nodes():
        ...
        if (icoor[0] % 2 == 1 and icoor[1] % 4 == 0) or (
            icoor[0] % 2 == 0 and icoor[1] % 4 == 2
        ):
            nb_coor = [
                [icoor[0] - 1, icoor[1] + 1],
                [icoor[0] + 1, icoor[1] + 1],
            ...
            for j in hexmap.nodes():
                ...
                if jcoor in nb_coor:
                    if (i, j) not in hexmap.edges():
                        hexmap.add_edge(i, j)
        elif (icoor[0] % 2 == 1 and icoor[1] % 4 == 3) or (
            icoor[0] % 2 == 0 and icoor[1] % 4 == 1
        ):
            nb_coor = [
                [icoor[0] - 1, icoor[1] - 1],
                [icoor[0] + 1, icoor[1] - 1],
    ...
    for y in range(n):  # for each row
        for x in range(n):
            ...
            for i in hexmap.nodes():
                icoor = hexmap.nodes[i]["coor"]
                if [x, y] == icoor:
                    hexmap.nodes[i]["colorcode"] = assign
...
def visualize_hexmap(hexmap, outfile):
    ...
    for node in hexmap.nodes():
        ...
        plt.plot(
            ...
            markersize=2,
            marker="o",
            color=colordict[hexmap.nodes[node]["colorcode"]],
        )
...
def assign_colors(G, source):
    ...
    for node in list(T.nodes()):
        ...
        used_color = [
            assigndict[n] for n in assigned_nbs + [node] if n in assigndict.keys()
        ]
        ...
        for nb in unassigned_nbs:
            ...
            avail_color.remove(
                assigndict[nb]
            )  # remove this color from available colors
        # print(assigndict)
    return T, assigndict
...
def place_assignment_on_hexmap(G, assigndict, T, hexmap):
    ...
    colorcode_1 = [
        node for node in hexmap.nodes() if hexmap.nodes[node]["colorcode"] == 1
    ]
    boundary = max([hexmap.nodes[node]["coor"][0] for node in hexmap.nodes()])
    center_locs = [
        node
        for node in colorcode_1
        if (
            hexmap.nodes[node]["coor"][0] <= 0.75 * boundary
            and hexmap.nodes[node]["coor"][0] >= 0.25 * boundary
        )
        and (
            hexmap.nodes[node]["coor"][1] <= 0.75 * boundary
            and hexmap.nodes[node]["coor"][1] >= 0.25 * boundary
        )
    ]
    # print(center_locs)
    fp = random.choice(
        center_locs
    )  # place the root cell in the randomly selected hexmap node
    ...
    for cell in list(T.nodes()):
        ...
        for nb in nbs:
            if nb not in placed_cells:
                # print(nb,'not placed')
                avail_locs = list(
                    hexmap.neighbors(placement_tmp.nodes[cell]["placement"])
                )
                # print('available locs', avail_locs)
                placement_tmp.nodes[nb]["placement"] = [
                    loc
                    for loc in avail_locs
                    if hexmap.nodes[loc]["colorcode"] == assigndict[nb]
                ][0]
...
def find_allowed_placement_on_hexmap(G, hexmap, source, steps):
    ...
    while allowed == False:
        ...
        locs = [
            placement_tmp.nodes[cell]["placement"] for cell in placement_tmp.nodes()
        ]
...
def place_assignment_on_hexmap_wbuffer(G, hexmap, source, steps):
    ...
    while allowed == False:
        ...
        center_locs = [
            ...
            for node in colorcode_1
            if (
                hexmap.nodes[node]["coor"][0] <= 0.6 * boundary
                and hexmap.nodes[node]["coor"][0] >= 0.4 * boundary
            )
            and (
                hexmap.nodes[node]["coor"][1] <= 0.6 * boundary
                and hexmap.nodes[node]["coor"][1] >= 0.4 * boundary
            )
        ]
        # print(center_locs)
        fp = random.choice(
        ...
        try:
            for cell in list(T.nodes()):
                ...
                nbs = list(
                    set(list(G2.neighbors(cell))) - set(placed_cells)
                )  # not placed neighbors
                print("neighbors", nbs)
                avail_locs = list(
                    set(list(hexmap.neighbors(placement_tmp.nodes[cell]["placement"])))
                    - set(occupied_hexlocs)
                )
                print("available locs", avail_locs)
                if len(avail_locs) >= len(
                    nbs
                ):  # if there is enough space to place the cell
                    for nb in nbs:
                        print(nb, "not placed")
                    ...
                    if len(avail_locs) != 0:
                        ...
                        occupied_hexlocs.append(placement)
                        buf_avail_locs = list(
                            set(list(hexmap.neighbors(placement)))
                            - set(occupied_hexlocs)
                        )
                        for nb in nbs:
                            print(nb, "not placed")
        ...
        if len(locs) == len(set(locs)) and len(locs) == len(
            list(placement_tmp.nodes())
        ):
            print("found placement solution")
            allowed = True
            break
...
def visualize_hexmap_placement_graphviz(hexmap, G_placement, outfile):
    ...
    for node in G_placement.nodes():
        ...
        plt.plot(
            ...
            markersize=2,
            marker="o",
            color=colordict[hexmap.nodes[placement]["colorcode"]],
        )
        plt.text(coor[0], coor[1], node)
    ...
    for edge in G_placement.edges():
        n0, n1 = edge[0], edge[1]
        c0, c1 = (
            hexmap.nodes[G_placement.nodes[n0]["placement"]]["coor"],
            hexmap.nodes[G_placement.nodes[n1]["placement"]]["coor"],
        )
        xlist = [c0[0], c1[0]]
        ylist = [c0[1], c1[1]]
        plt.plot(xlist, ylist, "k", zorder=0)
